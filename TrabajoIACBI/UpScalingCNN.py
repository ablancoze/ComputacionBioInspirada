...
class Subpixel(object):
    ...
    def build_model(self):
        ...
        if self.is_train:
            ...
            '''
            data = dataSetUtil.dataSet().load_data(self.is_train, self.test_img)
            input_ = dataSetUtil.dataSet().imread(data[0])       
    ...
    def __periodicShuffling (self, X, r):
    ...
    def model(self):
        ...
        ps = self.__periodicShuffling(conv3, self.scale)
        return tf.nn.tanh(ps)

    def train(self, config):
        dsu =  dataSetUtil.dataSet()
        # NOTE : if train, the nx, ny are ingnored
        dsu.input_setup(config)

        data_dir = dsu.checkpoint_dir(config)
        
        input_, label_ = dsu.read_data(data_dir)
        ...
        if config.is_train:
            ...
            x = np.squeeze(result)
            #dataSetUtil.dataSet().checkimage(x)
            print(x.shape)
            dataSetUtil.dataSet().imsave(x, config.result_dir+'/result2.png', config)


    def trainTensorFlow(self, config):
        dsu =  dataSetUtil.dataSet()
        # NOTE : if train, the nx, ny are ingnored
        dsu.input_setup_TensorFlow(config)
